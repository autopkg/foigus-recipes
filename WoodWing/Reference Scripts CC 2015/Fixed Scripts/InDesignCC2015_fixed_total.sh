...
CC_VERSION="2015"								# Creative Cloud version to install files for.
...
#	fi
	...
	CHOSEN_INSTANCE="/Applications/Adobe InDesign CC 2015/Adobe InDesign CC 2015.app"
...
function CheckInternalInstallationFolders()
{
	...
	# Install the PluginConfig.txt file to the application folder.
	cp -f "${BASE_PATH}/PluginConfig.txt" "${INSTALL_PATH}"
	...
	# ------------------------------------------------------------
	# Install the WoodWing ZXP folder.
	
	# Reset the install path.
	INSTALL_PATH=`dirname "$1"`
	
	# Check if the preceding folder path exists. If not, create them.
	WOODWINGZXP_PATH="${INSTALL_PATH}"
	
	mkdir -p "${WOODWINGZXP_PATH}"
	
	if [ -d "${WOODWINGZXP_PATH}" ]; then
		echo "Installing the WoodWing ZXP folder."
		echo "Copying '${BASE_PATH}/WoodWing ZXP' to '${WOODWINGZXP_PATH}'"
		
		# Install the WoodWing ZXP API and files.
		cp -rf "${BASE_PATH}/WoodWing ZXP" "${WOODWINGZXP_PATH}"
		GiveAllRightsToPath "${WOODWINGZXP_PATH}/WoodWing ZXP"
		
		# Check if the copy action succeeded.
		if [[ -d "${BASE_PATH}/WoodWing ZXP" ]]; then
			"${BASE_PATH}/WoodWing ZXP/mac_installer_CC.app/Contents/MacOS/mac_installer_CC"
		fi
		
		echo "Done."
	else
		echo "Failed to install the WoodWing ZXP API and files."
	fi
